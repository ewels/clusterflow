...
if(!defined($config{references}{bowtie})){
    ...
    warn "\nAligning against ".$config{references}{bowtie}."\n\n";
}
...
# See if we need to parse any parameters
my $mirna;
...
if($se_files && scalar(@$se_files) > 0){
	foreach my $file (@$se_files){
		...
		my $command = "$gzip bowtie -p $cores -t -m 1 $enc --strata --best -S --chunkmbs 2048 ".$config{references}{bowtie}." $file | samtools view -bS - > $output_fn";
		if ($mirna) {
		    $command = "$gzip bowtie -p $cores -t -n 0 -l 15 -e 99999 -k 200 $enc --best -S --chunkmbs 2048 ".$config{references}{bowtie}." $file | samtools view -bS - > $output_fn"
		}
		warn "\n###CFCMD $command\n\n";
...
if($pe_files && scalar(@$pe_files) > 0){
	foreach my $files_ref (@$pe_files){
		...
		if(scalar(@files) == 2){
			...
			my $command = "bowtie -p $cores -t -m 1 $enc --strata --best --maxins 700 -S --chunkmbs 2048 ".$config{references}{bowtie}." -1 ".$files[0]." -2 ".$files[1]." | samtools view -bSh 2>/dev/null - > $output_fn";

			if ($mirna) {
			    $command = "bowtie -p $cores -t -n 0 -l 15 -e 99999 -k 200 $enc --best --maxins 700 -S --chunkmbs 2048 ".$config{references}{bowtie}." -1 ".$files[0]." -2 ".$files[1]." | samtools view -bSh 2>/dev/null - > $output_fn";
			}
			warn "\n###CFCMD $command\n\n";
