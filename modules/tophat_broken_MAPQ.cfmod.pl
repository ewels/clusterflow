...
GTF path but requires a bowtie path.
...
if($se_files && scalar(@$se_files) > 0){
	foreach my $file (@$se_files){
		...
		my $command = "tophat -p $cores -g 1 $enc $gtf -o $output_dir ".$config{references}{bowtie}." $file";
		warn "\n###CFCMD $command\n\n";
...
if($pe_files && scalar(@$pe_files) > 0){
	foreach my $files_ref (@$pe_files){
		...
		if(scalar(@files) == 2){
			...
			my $command = "tophat -p $cores -g 1 $enc $gtf -o $output_dir ".$config{references}{bowtie}." ".$files[0]." ".$files[1];
			warn "\n###CFCMD $command\n\n";
